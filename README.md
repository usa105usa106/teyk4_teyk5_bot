...
## Elliott v6 strict engine update

В этой версии Elliott-модуль переделан так, чтобы анализ и график использовали одни и те же точки.

Правила:
- импульс рисуется только как 1-2-3-4-5, без повторов цифр;
- коррекция рисуется только как A-B-C;
- standalone A-B-C больше не считается VALID без предыдущего подтверждённого импульса 1-2-3-4-5;
- после 5 волн вверх ожидается A-B-C вниз;
- после 5 волн вниз ожидается A-B-C вверх;
- после полного 5+3 цикла бот может ждать новый импульс;
- POSSIBLE не рисуется как полноценная разметка и не влияет на сигнал;
- Elliott подтверждает сигнал только если структура VALID и направление совпадает.
